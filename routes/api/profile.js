...
router.get(
  ...
  (req, res) => {
    ...
    Profile.findOne({ user: req.user.id })
      ...
      .catch(err => res.status(404).json("error hai"));
  }
);
  ...
  Profile.find()
    ...
    .then(profiles => {
      if (!profiles) {
        ...
        Profile.countDocuments().then(count =>{
          data = {};
...
router.post('/search', (req, res) => {
  ...
  if(req.body.type == 'all'){
    Profile.find({
      $and:[
        {$or: 
          [{handle: { '$regex': `(\s+${req.body.key}|^${req.body.key})`, '$options': 'i' }},
           {username: { '$regex': `(\s+${req.body.key}|^${req.body.key})`, '$options': 'i' }}
          ]
        }
      ]
    ...
    .populate('user', ['name', 'avatar'])
      ...
      else if(req.body.type == 'ideaPitcher'){
        Profile.find({ $and:[
          {$or: 
            [{handle: { '$regex': `(\s+${req.body.key}|^${req.body.key})`, '$options': 'i' }},
             {username: { '$regex': `(\s+${req.body.key}|^${req.body.key})`, '$options': 'i' }}
